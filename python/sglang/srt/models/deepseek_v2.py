...
class DeepseekV2AttentionMLA(nn.Module):
    ...
    def dispatch_attn_forward_method(
        ...
        if self.attention_backend in ["flashinfer", "__hip_attention"]:
        ...
        elif self.attention_backend in ["hip_attention"]:
            ...
            if (
                ...
                if forward_batch.forward_mode.is_extend():
                    ...
                    return AttnForwardMethod.MHA_FROM_CACHE
                return AttnForwardMethod.MLA
        elif self.attention_backend in ["fa3", "__hip_attention"]:
            # Flash Attention: Use MHA with chunked KV cache when prefilling on long sequences.
            if forward_batch.extend_prefix_lens_cpu is not None:
                sum_extend_prefix_lens = sum(forward_batch.extend_prefix_lens_cpu)
            if (
    ...
    def forward(
        ...
        elif attn_forward_method == AttnForwardMethod.MHA_FROM_CACHE:
            return self.forward_normal_from_cache(
                positions, hidden_states, forward_batch
            )
        else:
            if _is_hip:
    ...
    def forward_absorb(
        ...
        if self.attention_backend in ["fa3", "flashinfer", "hip_attention"]:
